    ...
    <div class="max-w-7xl w-full h-full mx-auto p-6 lg:p-8 flex">
        <div class="m-auto w-full">
                    ...
                    <form method="POST" action="{{ route('link.create') }}">
                        <div class="w-full px-6 py-4 bg-white dark:bg-gray-800 shadow-md overflow-hidden rounded-lg">
                            ...
                            <div>
                                ...
                                <x-text-input id="long_url" class="block mt-1 w-full" type="text" name="long_url"
                                    :value="old('long_url')" required autofocus autocomplete="long_url" placeholder="Paste Your Long Url" />
                            ...
                            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                <div>
                                    ...
                                    <x-text-input id="custom_url" class="block mt-1 w-full" type="text"
                                        name="custom_url" :value="old('custom_url')" required autofocus
                                        autocomplete="custom_url" placeholder="{{'ex. type custom-url will be '.$url.'/custom-url'}}" />
                                    <x-input-error :messages="$errors->get('long-url')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="name" :value="__('Expired')" />
                                    <x-text-input id="expired" class="block mt-1 w-full" type="date"
                                        name="expired" :value="old('expired')" value="{{ $expired }}" required autofocus
            ...
            <div class="flex justify-center mt-16 px-0 sm:items-center sm:justify-center">
                <div class="ml-4 text-center text-sm text-gray-500 dark:text-gray-400 sm:text-right sm:ml-0 hover:text-blue-700 hover:cursor-pointer">
                    <a href="{{route('teams')}}">Kelompok 1 - K1 Teknik Informatika</a>
                </div>
            </div>
        </div>
    </div>
